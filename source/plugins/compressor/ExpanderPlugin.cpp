...
const std::string ExpanderPluginIdentifier(
    "com.nativeformat.plugin.compressor.expander");

constexpr char ExpanderPlugin::SidechainContentTypeKey[];
...
ExpanderPlugin::ExpanderPlugin(
  ...
  std::unique_ptr<Detector> expander_detector;
  ...
  if (grapher_node._knee_mode == NodeInfo::KneeMode::soft) {
    knee = SoftKnee<ExpanderType>();
  } else {
    knee = HardKnee<ExpanderType>();
  }

  Expander::Metadata meta = {_expander_params, knee,
                             std::move(expander_detector)};
  std::vector<Expander::Metadata> chains;
  chains.push_back(std::move(meta));

  _expanders.resize(bands, Expander(grapher_node._detection_mode, chains,
                                    _channels, _samplerate));
}

ExpanderPlugin::~ExpanderPlugin() {}

void ExpanderPlugin::feed(
  ...
  for (size_t band_index = 0; band_index < bands; ++band_index) {
    ...
    _expanders[band_index].compand(current_attack, current_release, time,
...
std::string ExpanderPlugin::name() { return ExpanderPluginIdentifier; }

bool ExpanderPlugin::finished(long sample_index, long sample_index_end) {
  return _child_plugin->finished(sample_index, sample_index_end);
}

void ExpanderPlugin::notifyFinished(long sample_index,
                                    long graph_sample_index) {
  return _child_plugin->notifyFinished(sample_index, graph_sample_index);
}

Plugin::PluginType ExpanderPlugin::type() const {
  return Plugin::PluginTypeConsumer;
}

void ExpanderPlugin::load(LOAD_CALLBACK callback) {
  _child_plugin->load(callback);
}

bool ExpanderPlugin::loaded() const { return _child_plugin->loaded(); }

void ExpanderPlugin::run(long sample_index, const NodeTimes &node_times,
                         long node_sample_index) {
  _child_plugin->run(sample_index, node_times, node_sample_index);
}

bool ExpanderPlugin::shouldProcess(long sample_index_start,
                                   long sample_index_end) {
  return _child_plugin->shouldProcess(sample_index_start, sample_index_end);
}

void ExpanderPlugin::split_bands(
